...
our @diploids = qw( Osat Hvul Bsta Bdis Barb Bsyl ); 
...
our @polyploids_labelled = (
   'Bhyb_A','Bhyb_B','Bhyb_C','Bhyb_D','Bhyb_E','Bhyb_F','Bhyb_G','Bhyb_H',
   'BmeP_A','BmeP_B','BmeP_C','BmeP_D','BmeP_E','BmeP_F','BmeP_G','BmeP_H',
   'BmeU_A','BmeU_B','BmeU_C','BmeU_D','BmeU_E','BmeU_F','BmeU_G','BmeU_H'
);
...
our %sister_clades = ();
$sister_clades{'MRCABarbBsyl'}{'MRCA1'} = ['Barb','Bsyl']; # clade 1
$sister_clades{'MRCABarbBsyl'}{'MRCA2'} = ['Barb','Bsyl']; # clade1 again with a different name, hack
...
our $MAXGAPSPERBLOCK = 250; # tolerated gaps for diploids in block
...
our @CODES = qw( A B C D E F H I all ); 
...
sub get_label_from_rules {
  ...
  if($anc_is_sister == 1 || $desc_dip_taxon eq ''){
    ...
    elsif($anc_dip_taxon eq 'Bdis' && $desc_dip_taxon eq 'MRCABarbBsyl'){
      $lineage_code = 'E'
    }
    #elsif($anc_dip_taxon eq 'Barb' && $desc_dip_taxon eq 'Bsyl'){
    #	$lineage_code = 'G'
    #}
    elsif($anc_dip_taxon eq 'MRCABarbBsyl' && $desc_dip_taxon eq 'Barb'){
      $lineage_code = 'F'
    }
    elsif($anc_dip_taxon eq 'MRCABarbBsyl' && $desc_dip_taxon eq 'Bsyl'){
      $lineage_code = 'H'
    }
    elsif($anc_dip_taxon eq 'Barb' && $desc_dip_taxon eq 'Bsyl'){
      $lineage_code = 'H'
    }
    elsif($anc_dip_taxon eq 'Bsyl' && $desc_dip_taxon eq 'Barb'){
      $lineage_code = 'F'

    }# swapped Bdis and Barb/Bsyl
    elsif($anc_dip_taxon eq 'MRCABarbBsyl' || $anc_dip_taxon eq 'Barb'
          || $anc_dip_taxon eq 'Bsyl'){
      if($desc_dip_taxon eq 'Bdis'){
        $lineage_code = 'D'
      }
    }
    elsif($anc_dip_taxon eq 'Bdis'){
      if($desc_dip_taxon eq 'MRCABarbBsyl' || $desc_dip_taxon eq 'Barb' 
